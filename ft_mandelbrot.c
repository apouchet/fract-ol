...
static void	ft_switch(t_data *data, int x, int y)
{
	if (data->fract == 0)
	{
		data->z_r = 0;
		data->z_i = 0;
		data->c_r = x / data->ratio_x + data->x_a;
		data->c_i = y / data->ratio_y + data->y_a;
	}
	else if (data->fract == 1)
	{
		data->z_r = x / data->ratio_x + data->x_a;
		data->z_i = y / data->ratio_y + data->y_a;
	}
}
	...
	while (x < FENETRE_X)
	{
		...
		while (y < FENETRE_Y)
		{
			...
			else
			{
				ft_switch(d, x, y);
				d->img[d->p.sl * y + x]
					= ft_color(ft_calcul_mdb_julia(d), d);
			}
			y += 2;
		}
		x += 2;
